...
procedure TfrExportacao.Memo1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
//   utilização de thread para importação (memoview)
     Memothread.Memo:=Memo1;
     Memothread.caminho:=path;
     Memothread.Resume;
     path:=EmptyStr;

// metodo de importação sem utilização de threads
// Memo1.Lines.Add()
 {   AssignFile(arq, path);
    Reset(arq);
    if (IOResult <> 0) then
